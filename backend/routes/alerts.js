...
//   if (!userAlerts || userAlerts.alerts.length === 0) return res.json([]);

//   const { alerts, deletedProductIds } = userAlerts;

//   const results = [];

//   for (let { productId, targetPrice } of alerts) {
//     if (deletedProductIds.includes(productId)) continue;

//     let product = await AmazonProduct.findById(productId).lean();
//     if (!product) product = await FlipkartProduct.findById(productId).lean();
//     if (!product) continue;
...
//     { userId },
//     {
//       $pull: { alerts: { productId } },
//       $addToSet: { deletedProductIds: productId }
//     },
//     { upsert: true }
//   );

//   res.status(200).json({ message: 'Alert deleted and product marked' });
// });
